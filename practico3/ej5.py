...
def monteCarlo0inf(fun):
    return lambda x : (1 / x**2) * fun(1/x - 1)
...
f_c = monteCarlo0inf(lambda x: (x * (1 + x**2)**(-2)))
f_d = monteCarlo0inf(lambda x: 2 * exp(-(x**2)))
...
for f in funciones:
    ...
    for N in [1000, 10000, 100000, 1000000]:
        print(f"iters: {N}, Integral entre {a} y {b} de {f} ~ {monteCarloIntegral(fun, a, b, N)}")
